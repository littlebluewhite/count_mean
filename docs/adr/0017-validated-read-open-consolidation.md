...
14. **codex r3 P2 — atomic commit 改 nested validated leaf anchor (Option A,user 核准 2026-05-31)**:codex round 3 抓到 pt13(b) 的 dirfd writer 對 **subdir target** 留有 commit-side TOCTOU —— tmp-create 走 `openat2 RESOLVE_NO_SYMLINKS` / `O_NOFOLLOW_ANY` 是 symlink-safe,但原 Commit 對多段 rel 走 `unix.Renameat(baseDirfd, "sub/…tmp", baseDirfd, "sub/…")`,而 renameat **沒有 `RESOLVE_NO_SYMLINKS` 等價**(`renameat2` flag 僅 `RENAME_{NOREPLACE,EXCHANGE,WHITEOUT}`)→ `sub` 在 tmp-create 後被換成 symlink 即可把最終 rename 導出 base(或 orphan 真 tmp)。**cross-check 揭 SubDir 是有測試的 handler feature**(`TestWritePhaseSyncResult_SubDir` / `TestWriteNormalizedPhaseSyncResult_SubDir` 斷言寫進 `OutputDir/SubDir/`、auto-mkdir;GUI 目前不接線、production 全 `io.WriteRequest{}`,但 contract 真實)→「fail-closed 拒 subdir」(評估過的 Option B)會 regress feature,故改採 **Option A:dirfd 從 base 改錨在 target 的「已驗證 leaf 父目錄」**。`OpenAtomicWriteValidated` 算 `relParent = Rel(hitBase, resolvedParent)`("." = 直屬 base);平台 `openAtomicWrite(baseDir, relParent, tmpBase, targetBase, …)` 先開 base、再以單一 `openat2(baseDirfd, relParent, RESOLVE_NO_SYMLINKS)` / `openat(O_NOFOLLOW_ANY)` 從 base **下行**到 leaf(`openLeafAnchor`)、關掉 base,leaf fd 即 anchor;tmp-create / renameat / unlinkat 全以 **basename** 相對該 pinned leaf。兩性質同時成立:(a) 下行從 base 開始、非重開絕對 resolvedParent → matchAnyBase 後 swap 進的 parent symlink 被 ELOOP 擋(codex P1,與 round-2 base-anchor 同等強度);(b) leaf fd PIN 住 inode、rename 以 basename → 無中間 component 可 swap(codex r3 P2)。**與 round-2 被否的「錨 resolvedParent」關鍵不同**:那是用絕對路徑重開(unsafe re-walk from /);這是從 base dirfd 經 NO_SYMLINKS 下行(safe)。新 red-green regression `SubdirCommitSurvivesParentSwap`(black-box,`!windows`):open 後把 real sub 移走、name `sub` 改指 base 外 evil,Commit 仍落在被 pin 的原 inode、evil 空 —— 51cb437 base-anchor 此時 renameat 經 swap 的 sub 找不到 tmp 回 ENOENT(紅),Option A 成功(綠)。round-2 anchor test 重寫為 `AnchorsSubdirTargetOnValidatedLeaf`(斷言 tmpRel/targetRel 為 basename);Linux resolve test 改測 leaf 下行 openat2 的 RESOLVE_NO_SYMLINKS(`relParent="linkdir"`)。ancient-kernel(openat2 ENOSYS)/ Windows / other-unix fallback 維持 `os.OpenFile`+`os.Rename` 全路徑(subdir parent-swap 殘餘 TOCTOU、與讀寫端一致、已知 deferred)。darwin(`O_NOFOLLOW_ANY`)+ Docker Linux(`openat2`)雙驗證綠;`internal/io` 的 `_SubDir` feature 測試保持綠。
