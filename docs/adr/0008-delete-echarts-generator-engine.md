...
3. `gui/app.go:73`（`chartGen *chart.EChartsGenerator` field）+ `:128`（`chartGen: chart.NewEChartsGenerator()`）—— constructed-never-called orphan，全 codebase 僅此 2 行提及。
4. self-referential 測試 4 檔：`echarts_generator_test.go`、`chart_statistics_test.go`、`connect_nulls_test.go`、`nil_empty_guards_test.go`（後者逐 case 確認**整檔**測 dead 符號，非 partial）。
