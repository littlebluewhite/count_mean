...
- **Impl process note — 與 [[ADR-0019]] 的協作順序（as-built：兩案平行落地、merge 時對齊）**：開工 cross-check 時 `filename.SubjectOutputName` 不存在（0019 當時 design-only），故先用 interim idiom `fmt.Sprintf("%s_normalized.csv", filename.Sanitize(subject))`。收尾 merge 時 0019 已平行落地（local main `730c040`，`SubjectOutputName` 存在並已 sweep csv_handler.go 其餘 5 個 Subject-based 寫入點）；因 0019 的 sweep 早於本 code 存在、無法觸及本 call site，遂於 merge 收斂時把本 call site 手動對齊為 `filename.SubjectOutputName(subject, "normalized") + ".csv"`——與 5 個 sibling 同 pattern、與 interim idiom **byte-identical**（`SubjectOutputName = Sanitize(subject) + "_" + suffix`）、既有 filename 斷言 test（`subject_01_normalized.csv`）鎖住。csv_handler.go 的 6 個 Subject-based 寫入點現全數一致走此 primitive，完成 Decision §2 的設計終態。
