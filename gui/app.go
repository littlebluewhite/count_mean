...
func (a *App) showNormalizationDialog() {
	...
	refFileBtn := widget.NewButton("選擇參考資料檔案", func() {
		a.showFileSelectDialog(&refFilePath, refFileLabel)
	})
	...
	// 創建按鈕
	executeBtn := widget.NewButton("執行標準化", func() {
		a.executeNormalization(mainFilePath, refFilePath, outputNameEntry.Text)
	})
	executeBtn.Importance = widget.HighImportance
