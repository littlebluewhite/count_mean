...
func testBackpressureConfig() *models.BackpressureConfig {
	return &models.BackpressureConfig{
		MaxMemoryMB:           1024,
		MaxWorkers:            4,
		MemoryThreshold:       0.8,
...
func TestBackpressureController_GetStats_ConcurrentRead_NoRace(t *testing.T) {
	bc := models.NewBackpressureController(testBackpressureConfig())
...
func TestBackpressureController_NormalizesNonpositiveInterval(t *testing.T) {
	...
	for _, tt := range cases {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			_ = models.NewBackpressureController(cfg)
		})
	}
}
