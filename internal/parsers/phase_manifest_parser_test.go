...
func TestGetPhaseValue(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(string(tt.phaseName), func(t *testing.T) {
			value, isIdx, err := GetPhaseValue(&phasePoints, tt.phaseName)
...
Subject003,motion003.csv,force003.anc,emg003.csv,130,NA,1.250,x,2.900,3.250,780,-,4.700,X,5.250`
		...
		// 測試分期點值獲取
		value, isIndex, err := GetPhaseValue(&m1.PhasePoints, models.PhaseD)
		require.NoError(t, err)
		assert.Equal(t, 780.0, value)
		assert.True(t, isIndex)

		value, isIndex, err = GetPhaseValue(&m1.PhasePoints, models.PhaseS)
		require.NoError(t, err)
