...
func TestAllPhases_ReturnsTenInCanonicalOrder(t *testing.T) {
	expected := []PhasePoint{
		PhaseP0, PhaseP1, PhaseP2, PhaseS, PhaseC,
		PhaseD, PhaseT0, PhaseT, PhaseO, PhaseL,
	}
	require.Equal(t, expected, AllPhases())
}
...
func TestAllPhases_IsDefensiveCopy(t *testing.T) {
	...
	first[0] = "MUTATED"
	second := AllPhases()
	assert.Equal(t, PhaseP0, second[0], "AllPhases must return a copy; mutating caller's slice leaked into internal storage")
}
...
func TestPhasePoint_IsValid(t *testing.T) {
	...
	invalid := []PhasePoint{"", "X", "P3", "p0", " P0", "Z9"}
...
func TestParsePhasePoint(t *testing.T) {
	t.Run("valid_canonical", func(t *testing.T) {
		for _, raw := range []string{"P0", "P1", "P2", "S", "C", "D", "T0", "T", "O", "L"} {
			got, err := ParsePhasePoint(raw)
			require.NoError(t, err, "valid %q should parse", raw)
			assert.Equal(t, PhasePoint(raw), got)
		}
	})
	...
	t.Run("invalid_rejected", func(t *testing.T) {
		...
		for _, raw := range invalid {
			...
			assert.Equal(t, PhasePoint(""), got, "invalid input should return zero value")
		}
	})
}
...
func TestPhasePoint_IsMotionIndex(t *testing.T) {
	...
	assert.False(t, PhasePoint("X").IsMotionIndex(), "invalid phase should not be motion index")
}
...
func TestPhasePoint_Order(t *testing.T) {
	...
	assert.Equal(t, -1, PhasePoint("X").Order(), "invalid phase order must be -1")
	assert.Equal(t, -1, PhasePoint("").Order(), "empty phase order must be -1")
}
...
func TestPhasePoint_JSON_RoundTrip(t *testing.T) {
	...
	for _, original := range AllPhases() {
...
func TestPhasePoint_UnmarshalJSON_RejectsInvalid(t *testing.T) {
	...
	for _, raw := range invalid {
		...
		assert.Equal(t, PhasePoint(""), decoded.Phase,
