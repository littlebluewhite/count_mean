...
func WriteFileNoFollow(path string, data []byte) (err error) {
	//nolint:gosec // G304: fsperm is a thin OpenFile wrapper; path validation is caller's responsibility
	f, err := os.OpenFile(path, WriteFlags, FilePerm)
	if err != nil {
		return fmt.Errorf("WriteFileNoFollow open: %w", err)
	}
	...
	if _, writeErr := f.Write(data); writeErr != nil {
		return fmt.Errorf("WriteFileNoFollow write: %w", writeErr)
	}
	return nil
}
