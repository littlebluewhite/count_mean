...
//
//  1. unix.Open(resolvedParent, O_DIRECTORY|O_CLOEXEC) 拿 validated 父目錄 dirfd
//  2. unix.Openat(dirfd, tmpBase, TmpCreateFlags|O_NOFOLLOW_ANY, FilePerm) 建 .tmp
...
//
// macOS 無 RESOLVE_BENEATH 等價;boundary 已在 caller-side matchAnyBase 完成
// (resolvedParent 確認在 base 下)。O_NOFOLLOW_ANY (macOS 12+) 讓 kernel 在 openat
// 階段拒絕「任一 component 為 symlink」,擋 EvalSymlinks→openat 之間 swap 進來的
// parent-component symlink TOCTOU,與 Linux RESOLVE_NO_SYMLINKS 對齊。
//
...
func openAtomicWrite(resolvedParent, tmpBase, targetBase, tmpPath, targetPath string) (*AtomicWriteHandle, error) {
	...
	fd, err := openatTmp(dirfd, tmpBase, unix.O_NOFOLLOW_ANY)
	if errors.Is(err, unix.EINVAL) {
		// macOS 11 / 更舊不認 O_NOFOLLOW_ANY → 移除 flag 重試 (dirfd 錨定保留)。
		fd, err = openatTmp(dirfd, tmpBase, 0)
	}
	if err != nil {
		...
		return nil, fmt.Errorf("openat(%s under %s, tmp-create): %w", tmpBase, resolvedParent, err)
	}
...
// openatTmp 以 dirfd 為錨點 + TmpCreateFlags|extraFlags|O_CLOEXEC 建立 tmpBase。
// extraFlags 用來在第一輪帶 O_NOFOLLOW_ANY、EINVAL fallback 第二輪帶 0。
func openatTmp(dirfd int, tmpBase string, extraFlags int) (int, error) {
	//nolint:wrapcheck // caller (openAtomicWrite) 已 wrap 帶 context;errors.Is 需透傳原始 errno
	return unix.Openat(dirfd, tmpBase, TmpCreateFlags|extraFlags|unix.O_CLOEXEC, uint32(FilePerm))
}
...
// unlinkatDir 以 dirfd 為錨點刪除 tmpBase。只在 dirfd 路徑被呼叫。
